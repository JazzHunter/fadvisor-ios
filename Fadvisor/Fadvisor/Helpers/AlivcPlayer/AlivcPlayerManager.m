...
- (void)setupPalyer
{
    ...
    _player.playAuthPreloadDefinition = @"SD";//720p
...
- (void)moveToVideoId:(NSString *)videoId uuid:(NSString *)uuid
{
    ...
    Weak(self);
    [self.playAuthService getVidPlayAuth:videoId completion:^(NSString *errorMsg, NSString *playAuth) {
        if (!weakself) {
            return;
        }
        // 错误处理
        ...
        [self.player moveTo:uuid playAuth:playAuth];
