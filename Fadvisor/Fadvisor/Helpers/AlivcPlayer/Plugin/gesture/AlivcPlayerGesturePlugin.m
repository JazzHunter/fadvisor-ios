    ...
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        if (self.isVerticalGesture) {
            ...
            if ([AlivcPlayerManager manager].duration) {
                BOOL moveProgress = (velocity.y >= self.containerView.height - 42);
                if (moveProgress) {
...
- (void)showSpeedSwiperAtDirection:(BOOL)right isChange:(BOOL)isChange {
    [self hideSpeedSwiper];
    self.speedSwiper.frame = CGRectMake((self.containerView.width - 130) / 2.0, 36, 130, 36);
...
//        _speedSwiper.accessibilityIdentifier = [self accessibilityId:@"speedSwiper"];
        _speedSwiper.backgroundColor = AlivcPlayerColor(@"vf_speed_bg");
    }
    return _speedSwiper;
}
