...
//        [self.replayLabel addGestureRecognizer:gestrue];
        ...
        CGFloat ladding = (self.width - 48 - 8 - 100 - 84 - 63) / 2.0;
...
//        _avatarView.accessibilityIdentifier = AUIVideoFlowAccessibilityStr(@"avatarView");
        _avatarView.image = [UIImage imageNamed:@"comment_avatar"];
...
//        _userLabel.accessibilityIdentifier = AUIVideoFlowAccessibilityStr(@"userLabel");
        ...
        _userLabel.textColor = FoundationColor(@"text_strong");
...
//        _userDescLabel.accessibilityIdentifier = AUIVideoFlowAccessibilityStr(@"userDescLabel");
        _userDescLabel.font = [UIFont systemFontOfSize:10 weight:UIFontWeightRegular];
...
//        _replayButton.accessibilityIdentifier = AUIVideoFlowAccessibilityStr(@"replayButton");
        _replayButton.backgroundColor = FoundationColor(@"tsp_fill_medium");
        _replayButton.titleLabel.font = [UIFont systemFontOfSize:12 weight:UIFontWeightRegular];
        [_replayButton setTitleColor:FoundationColor(@"ic_strong") forState:UIControlStateNormal];
...
- (void)updateBgImageView:(NSString *)urlstring
{
    ...
    [self.bgImageView setImageWithURL:[NSURL URLWithString:urlstring] transform:^UIImage * _Nullable(UIImage * _Nonnull image, NSURL * _Nonnull url) {
        [image imageByBlurSoft];
        [image imageByRoundCornerRadius:kBgImageBlurRadius];
        return image;
    }];
}
...
@property (nonatomic, strong) CustomImageButton *playButton;
...
- (CustomImageButton *)playButton
{
    if (!_playButton) {
        _playButton = [[CustomImageButton alloc] init];
//        _playButton.accessibilityIdentifier = AUIVideoFlowAccessibilityStr(@"playButton");
        ...
        [_playButton setImage:[UIImage imageNamed:@"player_paused"] forState:UIControlStateNormal];
...
- (void)updateButtonSize
{
    ...
    self.playButton.imageSize = size;
