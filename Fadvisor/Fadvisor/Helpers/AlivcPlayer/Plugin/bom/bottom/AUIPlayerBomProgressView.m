...
- (void)layoutSubviews
{
    ...
    self.leftTimeLabel.frame = CGRectMake(kPadding, 0, kTimeLabelWidth, self.height);
    self.rightTimeLabel.frame = CGRectMake(self.width - kTimeLabelWidth - kPadding, 0, kTimeLabelWidth, self.height);
    
    CGFloat contentWidth = self.rightTimeLabel.left - self.leftTimeLabel.right - kPadding;
    CGRect rect = CGRectMake(self.leftTimeLabel.right + kPadding /2, 0, contentWidth, self.height);
    self.progressView.frame = self.slider.frame = rect;
    self.progressView.height = 1;
    self.progressView.left -= 2;
    self.progressView.width -= 4;
...
//        _slider.accessibilityIdentifier = AUIVideoFlowAccessibilityStr(@"bom_slider");
        ...
        UIImage *image = [UIImage imageWithColor:APGetColor(APColorTypeCyanBg) size:CGSizeMake(28, 28)];
...
//        image = [image sd_roundedCornerImageWithRadius:14 corners:UIRectCornerAllCorners borderWidth:0 borderColor:nil];
        image = [image imageByRoundCornerRadius:14 corners:UIRectCornerAllCorners borderWidth:0 borderColor:nil borderLineJoin:kCGLineJoinRound];
...
//        _leftTimeLabel.accessibilityIdentifier = AUIVideoFlowAccessibilityStr(@"bom_leftTimeLabel");
        ...
        _leftTimeLabel.font = [UIFont systemFontOfSize:14 weight:UIFontWeightRegular];
...
//        _rightTimeLabel.accessibilityIdentifier = AUIVideoFlowAccessibilityStr(@"bom_rightTimeLabel");
        ...
        _rightTimeLabel.font = [UIFont systemFontOfSize:14 weight:UIFontWeightRegular];
