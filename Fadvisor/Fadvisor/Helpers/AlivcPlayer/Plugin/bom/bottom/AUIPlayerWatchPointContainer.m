...
//    CGFloat centerX =  [entry convertPoint:entry.center toView:self.superContainer].x;
    CGFloat centerX =  entry.center.x + self.left;
    
    if (centerX - view.bounds.size.width/2 < self.left) {
        centerX = view.bounds.size.width/2 + self.left;
    }
    
    if (centerX > self.superContainer.width - view.bounds.size.width/2 - self.left) {
        centerX = self.superContainer.width - view.bounds.size.width/2 - self.left;
    }
