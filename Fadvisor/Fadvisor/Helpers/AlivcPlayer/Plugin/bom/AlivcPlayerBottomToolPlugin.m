...
{
    if (!_bomView) {
        _bomView = [[AUIPlayerBomTool alloc] initWithFrame:CGRectMake(0, self.containerView.height - 42, self.containerView.width, 42)];
...
- (AUIPlayerSpeedTipView *)speedTipView {
    if (!_speedTipView) {
        _speedTipView = [[AUIPlayerSpeedTipView alloc] initWithFrame:CGRectMake(self.containerView.width - 23 - 145, 0, 145, 117)];
        _speedTipView.centerY = self.containerView.centerY;
...
- (void)updateUIHidden
{
    ...
    self.tipsLabel.frame = CGRectMake(12,self.containerView.height - self.tipsLabel.height -  (self.bomView.hidden?12: self.bomView.height), self.tipsLabel.width, self.tipsLabel.height);
...
{
    BOOL fullscreen = [AlivcPlayerManager manager].currentOrientation != 0;
    self.bomView.height = fullscreen ? 88 : 42;
    self.bomView.top = self.containerView.bounds.size.height - self.bomView.height;
    ...
    if (fullscreen) {
        self.thumbnailView.height = 166;
    } else {
        self.thumbnailView.height = 55;
    }
}
...
//    tipsLabel.accessibilityIdentifier = AUIVideoFlowAccessibilityStr(@"bottomTool_tipsLabel");
    ...
    tipsLabel.font = [UIFont systemFontOfSize:12 weight:UIFontWeightRegular];
    ...
    [self.tipsLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(self.containerView).mas_offset(-(self.bomView.hidden?12: self.bomView.height));
    }];
...
//    view.accessibilityIdentifier = AUIVideoFlowAccessibilityStr(@"landScapeSpeedView");
    ...
    view.onRateChanged = ^(float rate) {
        if (rate != [AlivcPlayerManager manager].rate) {
            ...
            [ToastView show:text view:weakSelf.containerView position:ToastViewPositionMid];
        }
    };
...
- (void)bomButtonViewDidClickHalfScreen {
    if ([AlivcPlayerManager manager].pageEventFrom == AlivcPlayerPageEventFromFullScreenPlayPage) {
        ...
        if ([AlivcPlayerManager manager].currentOrientation != 0) {
            ...
            [[Utils currentViewController].navigationController popViewControllerAnimated:YES];
        }
    }
}
