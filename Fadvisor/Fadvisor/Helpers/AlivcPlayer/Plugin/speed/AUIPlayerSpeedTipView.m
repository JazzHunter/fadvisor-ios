...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.tip.frame = CGRectMake(0, self.handImage.bottom + 7, self.tipImage.width, self.tipImage.height - self.handImage.bottom - 7);
        [self.tipImage addSubview:self.tip];
        self.handImage.centerX = self.tip.centerX;
        self.close.frame = CGRectMake(self.tipImage.right, 0, 28, 28);
...
- (UIImageView *)tipImage {
    if (!_tipImage) {
        ...
        _tipImage.image = [UIImage imageNamed:@"ic_speed_qipao"];
...
//        _handImage.accessibilityIdentifier = AUIVideoFlowAccessibilityStr(@"handImage");
        _handImage.image = [UIImage imageNamed:@"ic_speed_hand"];
    }
    return _handImage;
}
...
//        _tip.accessibilityIdentifier = AUIVideoFlowAccessibilityStr(@"speed_tip");
        _tip.font =  [UIFont systemFontOfSize:14 weight:UIFontWeightRegular];
        ...
        NSMutableAttributedString *attriStr = [[NSMutableAttributedString alloc] initWithString:@"长按屏幕\n快进" attributes:@{
            NSForegroundColorAttributeName: AlivcPlayerColor(@"vf_tip_text")
        }];
...
//        _close.accessibilityIdentifier = AUIVideoFlowAccessibilityStr(@"speed_close");
        [_close setImage:[UIImage imageNamed:@"ic_speed_close"] forState:UIControlStateNormal];
