...
- (void)getComments:(BOOL)isFromBottom completion:(void (^)(NSString *errorMsg, BOOL isHaveNewData))completion {
    ...
    [self GET:fetchCommentPageAPI parameters:params completion:^(BaseResponse *response) {
        ...
        // Set 形式
        if ([self.orderType isEqualToString:COMMENTS_ORDER_TYPE_SET]) {
            NSMutableArray<CommentSetModel *> *records = [CommentSetModel mj_objectArrayWithKeyValuesArray:response.responseObject[@"records"]];
//            if (records.count == 0 || total == records.count) {
//                self.noMore = YES;
//            }
            // 读取更多是插入到最后，否则是插入到最前面
            if (isFromBottom) {
                [self.commentSets addObjectsFromArray:records];
            } else {
                NSRange range = NSMakeRange(0, records.count);
                NSIndexSet *set = [NSIndexSet indexSetWithIndexesInRange:range];
                [self.commentSets insertObjects:records atIndexes:set];
            }
...
- (void)reset {
    self.commentSets = [NSMutableArray array];
