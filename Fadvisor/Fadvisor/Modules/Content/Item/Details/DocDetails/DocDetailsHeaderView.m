...
- (void)initUI {
    ...
    _titleLabel.myHeight = ListTitleFontSize * 2.5;
    ...
    _coverImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, CoverImgHeight, CoverImgHeight)];
...
//    [_coverImage setCornerRadius:4];
    _coverImage.leftPos.equalTo(_titleLabel.leftPos);
    _coverImage.topPos.equalTo(_titleLabel.bottomPos).offset(6);
    [self addSubview:_coverImage];

    UIImageView *sizeIcon = [self createIconImage:@"ic_data"];
    sizeIcon.leftPos.equalTo(_coverImage.rightPos).offset(8);
    sizeIcon.centerYPos.equalTo(_coverImage.centerYPos);
...
- (void)setModel:(ItemModel *)model {
    _coverImage.image = [UIImage imageNamed:[NSString stringWithFormat:@"doc_%@", model.fmt]];
...
//    _titleLabel.text = model.title;
    ...
    // TODO
    _sizeLabel.text = [NSString stringWithFormat:@"%ld", model.size];
    [_sizeLabel sizeToFit];
    
    _pageLabel.text = [NSString stringWithFormat:@"%ld", model.pageSize];
    [_pageLabel sizeToFit];
    
    _downloadLabel.text = [NSString stringWithFormat:@"%ld", (long)model.downloadCount];
