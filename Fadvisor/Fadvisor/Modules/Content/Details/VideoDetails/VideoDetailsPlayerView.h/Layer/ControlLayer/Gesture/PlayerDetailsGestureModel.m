...
@interface PlayerDetailsGestureModel ()<UIGestureRecognizerDelegate>
/*
 * 功能 ： 单击手势
 */
...
@implementation PlayerDetailsGestureModel
...
static GestureOrientation moveOrientation = GestureOrientationUnknow;
...
- (void)drapGesture:(UIPanGestureRecognizer *)sender {
    ...
    switch (sender.state) {
        case UIGestureRecognizerStateBegan: {
            if (moveOrientation == GestureOrientationUnknow) {
                ...
                if (self.delegate && [self.delegate respondsToSelector:@selector(gestureModel:state:moveOrientation:)]) {
                    [self.delegate gestureModel:self state:sender.state moveOrientation:moveOrientation];
                }
            }
            break;
        }
        case UIGestureRecognizerStateChanged: {
            if (moveOrientation == GestureOrientationVertical) {//垂直
                if (self.isLock == YES) {
                    return;// 锁屏不可以手势控制
                }
                ...
                //记录上一次移动点
                _savePoint = point;
            } else if (moveOrientation == GestureOrientationHorizontal) {//水平
//                //水平偏移量  这个功能禁止
        ...
        case UIGestureRecognizerStateFailed:{
            ...
            moveOrientation = GestureOrientationUnknow;
            break;
        }
        default:
...
- (void)gestureRecognizerStateBeganWithpoint:(CGPoint)point velocity:(CGPoint)velocity {
    ...
    if (isVerticalGesture) {
        moveOrientation = GestureOrientationVertical;    //垂直
        ...
        moveOrientation = GestureOrientationHorizontal;  //水平
    }
}
...
- (void)gestureRecognizerStateChangedWithPoint:(CGPoint)point view:(UIView *)view savePoint:(CGPoint)savePoint {
    ...
    if (direction == UISwipeGestureRecognizerDirectionLeft) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(gestureModel:brightnessDirection:)]) {
            [self.delegate gestureModel:self brightnessDirection:tempDirection];
        }
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(gestureModel:volumeDirection:)]) {
            [self.delegate gestureModel:self volumeDirection:tempDirection];
        }
    }
}
