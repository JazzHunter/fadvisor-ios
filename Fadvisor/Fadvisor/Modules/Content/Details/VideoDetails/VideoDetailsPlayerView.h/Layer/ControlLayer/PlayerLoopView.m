...
- (CGSize)screenSize {
    ...
    if (appScreenSize.width == 0 || lastOrientation != orientation) {
        ...
        if (orientation == UIDeviceOrientationLandscapeLeft || orientation == UIDeviceOrientationLandscapeRight) {
            ...
            appScreenSize.height = self.width;
        } else {
...
- (void)setupView {
    self.tickerLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.width, self.height)];
...
- (void)animateCurrentTickerString {
    ...
    float startingX = self.width;
    _Speed = (self.width + textSize.width) / 5;
    ...
    float duration = (textSize.width + self.width) / _Speed + 2;
...
- (void)statusBarOrientationChange:(NSNotification *)notification {
    ...
    if (orientation == UIInterfaceOrientationLandscapeRight || orientation == UIInterfaceOrientationLandscapeLeft) {
        //横屏
        self.frame = CGRectMake(0, self.frame.origin.y, [self screenSize].height, self.height);
    } else if (orientation == UIInterfaceOrientationPortrait ||
               orientation == UIInterfaceOrientationPortraitUpsideDown) {
        //竖屏
        self.frame = CGRectMake(0, self.frame.origin.y, [self screenSize].width, self.height);
    }
}
