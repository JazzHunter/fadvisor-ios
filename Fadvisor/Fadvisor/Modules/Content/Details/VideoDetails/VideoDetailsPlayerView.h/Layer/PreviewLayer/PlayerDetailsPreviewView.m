...
@interface PlayerDetailsPreviewView ()
...
@implementation PlayerDetailsPreviewView
...
- (UIButton *)replayButton {
    if (!_replayButton) {
        ...
        [_replayButton setImage:[UIImage imageNamed:@"ic_relaod"] forState:UIControlStateNormal];
...
- (void)replay {
    self.hidden = YES;
    if (self.delegate && [self.delegate respondsToSelector:@selector(previewViewReplay)]) {
        [self.delegate previewViewReplay];
    }
}

- (void)goVipController {
    if (self.delegate && [self.delegate respondsToSelector:@selector(previewViewGoVipController)]) {
        [self.delegate previewViewGoVipController];
    }
}

- (void)goBack {
    if (self.delegate && [self.delegate respondsToSelector:@selector(previewViewGoBack)]) {
        [self.delegate previewViewGoBack];
    }
}
