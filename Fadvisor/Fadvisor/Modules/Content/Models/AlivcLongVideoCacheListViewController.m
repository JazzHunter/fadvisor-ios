...
- (UIButton *)backButton {
    if (!_backButton) {
        ...
        [_backButton setImage:[UIImage imageNamed:@"ic_left_arr"] forState:UIControlStateNormal];
    }
...
- (void)deleteDownloadVideo {
    if (self.editArrayModel.editVideoArray.count == 0) {
        [MBProgressHUD showMessage:[@"请选择至少一个视频" localString] ToView:self.view];
    } else {
...
- (void)changeToPlayLocalVideo:(AlivcLongVideoDownloadSource *)video {
    if (video.downloadStatus != LongVideoDownloadTypefinish) {
        [MBProgressHUD showMessage:[@"视频还未下载完成" localString] ToView:self.view];
        return;
    }
    //不熄屏
